...
def main():
    ...
    controller = MultiSequenceController(model, kp=150, kd=15)
    ...
    # 序列2: 执行第一个动作
    sequence_1 = [0.5, -0.3, 0.2, 0.1, -0.2, 0.1]
    
    # 序列3: 执行第二个动作
    sequence_2 = [0.8, -0.5, 0.4, 0.3, -0.4, 0.3]
    
    # 序列4: 执行第三个动作
    sequence_3 = [1.0, -0.7, 0.6, 0.5, -0.6, 0.5]
    
    # 添加所有序列到控制器
    controller.add_sequence(home_position, duration=2.0)
    controller.add_sequence(sequence_1, duration=3.0)
    controller.add_sequence(sequence_2, duration=3.0)
    controller.add_sequence(sequence_3, duration=3.0)
    controller.add_sequence(home_position, duration=2.0)  # 最后回到初始位置
    
    print(f"已添加 {len(controller.sequences)} 个动作序列")
    
    # 启动仿真
    with mujoco.viewer.launch_passive(controller.model, controller.data) as viewer:
        print("仿真开始! 按Ctrl+C退出")
        try:
